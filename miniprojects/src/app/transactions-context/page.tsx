...
export default function Transactions() {
    ...
    const onSelectedTransaction = useCallback((clickedTransaction: Transaction) => {
        const selectedTransaction = transactions.filter((transaction) => transaction.id === clickedTransaction.id);
        setSelectedTransactions(selectedTransaction);
    }, [transactions]);

    const onDeletedTransaction = useCallback((removedTransaction: Transaction) => {
        const remainingTransactions = transactions.filter((transaction) => removedTransaction.name !== transaction.name);
            ...
            <div className="transactions-results">
                <h3>Selected transaction</h3>
                {
                    selectedTransactions.map((transaction, index) => (
                        <SelectedTransactionComponent getTransaction={transaction} key={index} />
                    ))
                }
            </div>
            <br />
